...
namespace Collections
{
    ...
    class Program
    {
        ...
        static void Main()
        {
            ...
            var result1 = debtors.Where(x=> x.Email.Contains("rhyta.com") || x.Email.Contains("dayrep.com")).ToList();


            //2) Yashi 26 - dan 36 - ya qeder olan borclulari cixartmag

            var today = DateTime.Now.Year;
            var result2 = debtors.FindAll(x=> today - x.BirthDay.Year >= 26 && today - x.BirthDay.Year <=36);

            //3) Borcu 5000 - den cox olmayan borclularic cixartmag

            var result3 = debtors.FindAll(x => x.Debt <= 5000);

            //4) Butov adi 18 simvoldan cox olan ve telefon nomresinde 2 ve ya 2 - den cox 7 reqemi olan borclulari cixartmaq

            var result4 = debtors.Where(x=> x.FullName.Length >=18 && x.Phone.Count(i=> i==7) >= 2).ToArray();

            //5) Qishda anadan olan borclulari cixardmaq

            var result5 = debtors.FindAll(x => x.BirthDay.Month == 1 || x.BirthDay.Month == 2 || x.BirthDay.Month == 12);
            ...
            double avrgeDebt = totalDebt / debtors.Count;

            var morethanavrge = debtors.FindAll(x => x.Debt >= avrgeDebt);
            var result6Fulname = morethanavrge.Select(x => x.FullName).ToList();
            result6Fulname.Sort();
            foreach (var item in result6Fulname)
            {
                //Console.WriteLine(item);
            }


            var result6 =  morethanavrge.OrderBy(x => x.Debt).Reverse();
            ...
            var result7 = debtors.FindAll(x => !x.Phone.Contains("8"));

            foreach (var item in result7)
            {
               Console.WriteLine($"{item.FullName}   {DateTime.Now.Year-item.BirthDay.Year}  {item.Debt}");
            }
            ...
            var max5debt = debtors.OrderBy(x => x.Debt).Reverse().ToArray();

            for(int i=0; i<5; i++)
            {
               // Console.WriteLine(max5debt[i].Debt);
            }


            //11)Butun borcu olanlarin borcunu cemleyib umumi borcu cixartmaq

            var totaldebtor = debtors.Select(x => x.Debt);
            var result11 = totaldebtor.Sum();
            ...
            var worldWar =new DateTime(1945, 9, 2);
            var result12 = debtors.FindAll(x => x.BirthDay <= worldWar);
            ...
            var result14 = debtors.FindAll(x => x.Debt -((12- Math.Abs( x.BirthDay.Month - DateTime.Now.Month))*500) >=0);
            foreach (var item in debtors)
            {
                var diffrnc=0;
                if (item.BirthDay.Month >= DateTime.Now.Month)
                {
                     diffrnc = item.BirthDay.Month - DateTime.Now.Month;
                  
                }

                else
                {
                     diffrnc = 12- DateTime.Now.Month + item.BirthDay.Month;
                 
                }

                var rmDebt = item.Debt - (diffrnc * 500);
            ...
            var result15 = debtors.FindAll(x => x.FullName.Contains("s") && x.FullName.Contains("m") && x.FullName.Contains("i") && x.FullName.Contains("l") && x.FullName.Contains("e"));
        }
